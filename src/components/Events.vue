      ...
      <div class="col-12 row g-3">
        <div class="col-6 col-md-3" v-for="filter in filtersList" :key="filter.key">
          ...
          <div v-else-if="filter.type === 'range'">
            ...
            <input type="range" min="5" max="50" step="5" v-model="filters.distance" class="form-range" />
    ...
    <div class="row">
      ...
      <div class="col-12 col-lg-4 d-flex flex-column gap-4">
        <div
          ...
          :key="program.id"
          :class="['card', 'shadow-sm', 'hover-card', selectedIndex === index ? 'card-selected' : '']"
        ...
        >
          <div class="card-body d-flex flex-column">
            ...
            <p class="card-text small text-muted">
              📅 {{ program.date }} • ⏰ {{ program.time }} • 🧑‍🤝‍🧑 {{ program.rsvps }}/{{ program.capacity }} attendees
            </p>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    searchEvents() {
      ...
      this.filteredPrograms = this.programs.filter((event) => {
        ...
        const distanceMatch = this.getDistance(
